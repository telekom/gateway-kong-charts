...
Set `adminApi.tls.enabled=true` to use HTTPS for the Admin API listener and chart-managed Admin API clients. The chart does not yet support custom Admin API certificates or CA trust for this path, so Kong uses an autogenerated self-signed certificate for `localhost`. The setup jobs do not trust its issuer and connect through a different Admin API Service hostname. With the secure default `adminApi.tls.insecure=false`, the jobs and Kong's internal Admin API route therefore fail closed. Set `adminApi.tls.insecure=true` only when you accept encrypted but unauthenticated in-cluster connections until proper certificate support is available.

A custom `configuration` script replaces the chart's default script. Its job receives `CURL_OPTS`, but curl does not read this variable automatically. Every curl call in the custom script must invoke `curl $CURL_OPTS` for `adminApi.tls.insecure` to take effect.
...
| adminApi.tls.enabled | bool | `false` | Enable HTTPS for the Admin API |
| adminApi.tls.insecure | bool | `false` | Skip certificate and hostname verification for chart-managed Admin API clients. The chart does not yet support custom Admin API certificates or CA trust. Kong therefore uses a self-signed certificate for localhost, which setup jobs cannot verify through the Admin API Service hostname. |
...
| issuerService.image | object | `{"repository":"issuer-service","tag":"2.3.5"}` | Issuer Service image configuration (inherits from global.image) |
...
| kedaAutoscaling.advanced.horizontalPodAutoscalerConfig | object | `{"behavior":{"scaleDown":{"policies":[{"periodSeconds":300,"type":"Percent","value":10}],"selectPolicy":"Min","stabilizationWindowSeconds":300},"scaleUp":{"policies":[{"periodSeconds":60,"type":"Percent","value":100},{"periodSeconds":60,"type":"Pods","value":4}],"selectPolicy":"Max","stabilizationWindowSeconds":0}}}` | HPA behavior configuration (scale-up/scale-down policies) |
| kedaAutoscaling.advanced.horizontalPodAutoscalerConfig.behavior | object | `{"scaleDown":{"policies":[{"periodSeconds":300,"type":"Percent","value":10}],"selectPolicy":"Min","stabilizationWindowSeconds":300},"scaleUp":{"policies":[{"periodSeconds":60,"type":"Percent","value":100},{"periodSeconds":60,"type":"Pods","value":4}],"selectPolicy":"Max","stabilizationWindowSeconds":0}}` | Scaling behavior policies |
| kedaAutoscaling.advanced.horizontalPodAutoscalerConfig.behavior.scaleDown | object | `{"policies":[{"periodSeconds":300,"type":"Percent","value":10}],"selectPolicy":"Min","stabilizationWindowSeconds":300}` | Scale-down behavior (conservative to prevent flapping) |
| kedaAutoscaling.advanced.horizontalPodAutoscalerConfig.behavior.scaleDown.policies | list | `[{"periodSeconds":300,"type":"Percent","value":10}]` | Scale-down policies (multiple policies can be defined) |
...
| kedaAutoscaling.triggers.prometheus.query | string | `"max_over_time((sum(rate(kong_http_requests_total{ei_telekom_de_zone=\"{{ .Values.global.zone }}\",ei_telekom_de_environment=\"{{ .Values.global.environment }}\",app_kubernetes_io_instance=\"{{ .Release.Name }}-kong\"}[1m])))[15m:30s])"` | PromQL query to execute Must return a single numeric value Can use Helm template variables (e.g., {{ .Values.global.zone }}) Example queries:   - Request rate: sum(rate(kong_http_requests_total{zone="zone1"}[1m]))   - Error rate: sum(rate(kong_http_requests_total{status=~"5.."}[1m])) Wrapped in max_over_time with a 15m/30s subquery so that brief metric pipeline outages (e.g. VM agent down) returning zero/low values do not trigger a scale-down. Scale-up is unaffected because the latest peak is always included in the max. |
...
| kedaAutoscaling.triggers.prometheus.timeout | string | `""` | HTTP timeout for Prometheus requests (optional) Supports milliseconds as a number or a duration such as "10s" Empty uses KEDA's default of 300ms, or KEDA_HTTP_DEFAULT_TIMEOUT configured on the controller |
...
| ssl.ciphers | string | `"DHE-DSS-AES128-GCM-SHA256:DHE-DSS-AES256-GCM-SHA384:DHE-RSA-AES128-CCM:DHE-RSA-AES256-CCM:DHE-RSA-AES128-SHA256:DHE-RSA-AES256-SHA256:DHE-RSA-AES128-GCM-SHA256:DHE-RSA-AES256-GCM-SHA384:DHE-RSA-CHACHA20-POLY1305:ECDHE-ECDSA-AES128-CCM:ECDHE-ECDSA-AES256-CCM:ECDHE-ECDSA-AES128-SHA256:ECDHE-ECDSA-AES256-SHA384:ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-AES128-SHA256:ECDHE-RSA-AES256-SHA384:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-RSA-CHACHA20-POLY1305:TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256:TLS_AES_128_CCM_SHA256"` | Custom TLS ciphers (OpenSSL format, ignored unless cipherSuite is 'custom') |
